...
class KanelaAgentBuilder {
    ...
    public void addTypeTransformation(TypeTransformation typeTransformation) {
        if (typeTransformation.isActive()) {
            if (config.getInstrumentationRegistryConfig().isEnabled()) {
                InstrumentationRegistry.instance().register(moduleDescription, typeTransformation);
            }
    ...
    private AgentBuilder.Listener additionalListeners() {
        ...
        if (config.getInstrumentationRegistryConfig().isEnabled()) listeners.add(InstrumentationRegistry.instance());
